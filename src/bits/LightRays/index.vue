...
const initWebGL = async () => {
  if (!containerRef.value || !isVisible.value || isInitialized.value) return
...
onMounted(() => {
  ...
  // 立即设置为可见，因为 Header 在首屏
  isVisible.value = true
  initWebGL()

  // 仍然设置 observer 用于处理页面滚动后的可见性变化
  observer = new IntersectionObserver(
    (entries) => {
      const newIsVisible = entries[0].isIntersecting
      if (newIsVisible && !isInitialized.value) {
        isVisible.value = true
        initWebGL()
      } else if (!newIsVisible && isInitialized.value) {
        cleanup()
...
watch(() => props.raysOrigin, () => {
  cleanup()
  setTimeout(initWebGL, 10)
})

watch(() => props.raysColor, (v) => {
  if (uniforms) uniforms.raysColor.value = hexToRgb(v)
})

watch(() => props.raysSpeed, (v) => {
  if (uniforms) uniforms.raysSpeed.value = v
})

watch(() => props.lightSpread, (v) => {
  if (uniforms) uniforms.lightSpread.value = v
})

watch(() => props.rayLength, (v) => {
  if (uniforms) uniforms.rayLength.value = v
})

watch(() => props.pulsating, (v) => {
  if (uniforms) uniforms.pulsating.value = v ? 1.0 : 0.0
})

watch(() => props.fadeDistance, (v) => {
  if (uniforms) uniforms.fadeDistance.value = v
})

watch(() => props.saturation, (v) => {
  if (uniforms) uniforms.saturation.value = v
})

watch(() => props.mouseInfluence, (v) => {
  if (uniforms) uniforms.mouseInfluence.value = v
})

watch(() => props.noiseAmount, (v) => {
  if (uniforms) uniforms.noiseAmount.value = v
})

watch(() => props.distortion, (v) => {
  if (uniforms) uniforms.distortion.value = v
})
...
<template>
  <div
    ref="containerRef"
    :class="`w-full h-full pointer-events-none z-[3] overflow-hidden relative ${className}`.trim()"
  />
</template>
